...
class SettlementService
{
    ...
    public function __construct(
        ...
        $this->orderService =
            $orderService ?? new OrderService(null, $this->repository);
        $this->discountService =
            $discountService ??
            new DiscountService(null, $this->repository, $this->orderService);
    }
    ...
    public function settle(array $data, string $source = "webhook"): array
    {
        ...
        if (!$this->orderService->isFlizPayOrder($orderData)) {
            return $this->finish(
                ...
                "Order was not placed with FLIZpay",
                $source,
                $norm,
            );
        }

        $txRow = $this->repository->getTransactionByTxId(
            $norm["transactionId"],
        );
        if ($txRow === null || (int) $txRow->kBestellung !== $norm["orderId"]) {
            return $this->finish(
                ...
                "Transaction was not issued for this order",
                $source,
                $norm,
            );
        }
        ...
        if ($norm["originalAmount"] !== $snapshotAmount) {
            return $this->finish(
                false,
                "amount_mismatch",
                "originalAmount differs from the issued snapshot",
                $source,
                $norm,
            );
        }
        if ($norm["currency"] !== \strtoupper((string) $txRow->cCurrency)) {
            return $this->finish(
                false,
                "currency_mismatch",
                "Currency differs from the issued snapshot",
                $source,
                $norm,
            );
        }
        ...
        if ($norm["status"] === "pending" || $norm["status"] === "processing") {
            $this->repository->updateTransactionStatus(
                $norm["transactionId"],
                $norm["status"],
            );
        ...
        if ($orderRow === null) {
            return $this->finish(
                false,
                "internal_error",
                "FLIZpay order state row missing",
                $source,
                $norm,
            );
        }
        ...
        return $this->settleFailedOrCanceled(
            $norm,
            $txRow,
            $orderData,
            $orderRow,
     ...
     */
    private function settleCompleted(
        array $norm,
        object $orderData,
        object $orderRow,
        string $source,
        ...
        if (!$this->repository->markPaidOnce($orderId, $txId)) {
            ...
            if (
                $freshRow === null ||
                (string) $freshRow->cCompletedTx !== $txId
            ) {
                return $this->finish(
                    true,
                    "already_paid",
                    "Another transaction completed this order",
                    $source,
                    $norm,
                );
            }
            ...
            if (!$this->repository->claimStaleCompletion($orderId, $txId)) {
                return $this->finish(
                    true,
                    "in_progress",
                    "Another settlement is booking this payment",
                    $source,
                    $norm,
                );
            }
        ...
        try {
            $discount = \round(
                (float) $norm["originalAmount"] - (float) $norm["amount"],
                2,
            );
            if ($discount >= 0.005) {
            ...
            return $this->finish(
                false,
                "booking_failed",
                "Booking failed after payment confirmation",
                $source,
                $norm,
            );
        }
    ...
    private function settleFailedOrCanceled(
        ...
        if (
            $orderData->dBezahltDatum !== null ||
            (int) $orderRow->nPaid === 1
        ) {
            return $this->finish(
                true,
                "already_paid",
        ...
        $marker =
            $norm["status"] === "failed"
                ? (string) $orderRow->cFailedTx
                : (string) $orderRow->cCanceledTx;
        if ($marker === $txId) {
            return $this->finish(
                true,
                "duplicate",
        ...
        $won =
            $norm["status"] === "failed"
                ? $this->repository->markFailedOnce(
                    $orderId,
                    $txId,
                    (int) $txRow->nAttempt,
                )
                : $this->repository->markCanceledOnce(
                    $orderId,
                    $txId,
                    (int) $txRow->nAttempt,
                );
        if (!$won) {
            return $this->finish(
                true,
                "duplicate",
                "Concurrent settlement already processed this event",
                $source,
                $norm,
            );
        }
    ...
    private function normalize(array $data): ?array
    {
        ...
        if (!\is_numeric($orderId) || (int) $orderId <= 0) {
            return $this->rejectPayload("orderId", $data);
        }
        $transactionId = $data["transactionId"] ?? null;
        if (!\is_string($transactionId) || \trim($transactionId) === "") {
            return $this->rejectPayload("transactionId", $data);
        }
        $status = \strtolower(\trim((string) ($data["status"] ?? "")));
        if ($status === "") {
            return $this->rejectPayload("status", $data);
        }
        foreach (["amount", "originalAmount"] as $field) {
            if (!isset($data[$field]) || !\is_numeric($data[$field])) {
                return $this->rejectPayload($field, $data);
            }
        }
        $currency = \strtoupper(\trim((string) ($data["currency"] ?? "")));
        if (\strlen($currency) !== 3) {
            return $this->rejectPayload("currency", $data);
        }
    ...
    /**
     * Names the failing field (never its value) and which keys arrived.
     */
    private function rejectPayload(string $field, array $data): ?array
    {
        FlizPlugin::debug("settlement: payload field invalid", [
            "field" => $field,
    ...
    private function finish(
        ...
        FlizPlugin::debug("settlement: amounts", [
            "order" => $context["orderId"] ?? null,
            "tx" => $context["transactionId"] ?? null,
            "result" => $result,
            "amount" => $context["amount"] ?? null,
            "originalAmount" => $context["originalAmount"] ?? null,
